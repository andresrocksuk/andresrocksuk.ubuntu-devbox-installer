...
# This script installs k9s Kubernetes CLI from GitHub releases
...
SOFTWARE_NAME="k9s"
SOFTWARE_DESCRIPTION="k9s (Kubernetes CLI)"
COMMAND_NAME="k9s"
...
install_k9s() {
    ...
    local download_url
    # k9s uses different naming: k9s_Linux_amd64.tar.gz
    download_url=$(echo "$release_info" | grep '"browser_download_url":' | grep "k9s_Linux_${arch}" | grep "\.tar\.gz\"" | head -n1 | sed -E 's/.*"([^"]+)".*/\1/')
    ...
    if [ "$FRAMEWORK_AVAILABLE" = "true" ] && command -v create_secure_temp_dir >/dev/null 2>&1; then
        temp_dir=$(create_secure_temp_dir "k9s")
    else
        temp_dir=$(mktemp -d)
        chmod 700 "$temp_dir"
    fi
    ...
    # Download k9s archive using framework if available
    ...
    if [ ! -f "$temp_dir/k9s" ]; then
        log_error "k9s binary not found in extracted archive"
        rm -rf "$temp_dir"
        return 1
    fi
    
    log_info "Installing k9s..."
    sudo mv "$temp_dir/k9s" "/usr/local/bin/$COMMAND_NAME"
    ...
    if [ "$FRAMEWORK_AVAILABLE" = "true" ] && command -v verify_installation >/dev/null 2>&1; then
        if verify_installation "$COMMAND_NAME" "$COMMAND_NAME --help" "help"; then
