...
TOOL_VERSION="v0.17.0"
TOOL_DESCRIPTION="Generate documentation for Terraform modules"
...
install_terraform_docs() {
    ...
    if command -v terraform-docs >/dev/null 2>&1; then
        ...
        current_version=$(terraform-docs --version | head -n1 | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    ...
    local download_url="https://github.com/terraform-docs/terraform-docs/releases/download/${TOOL_VERSION}/terraform-docs-${TOOL_VERSION}-linux-amd64.tar.gz"
    local archive_file="$temp_dir/terraform-docs.tar.gz"
    ...
    # Install binary
    local binary_path="$temp_dir/terraform-docs"
    if [ ! -f "$binary_path" ]; then
        log_error "terraform-docs binary not found in extracted archive"
        rm -rf "$temp_dir"
        return 1
    fi
    ...
    if ! sudo mv "$binary_path" "/usr/local/bin/terraform-docs"; then
    ...
    if command -v terraform-docs >/dev/null 2>&1; then
        ...
        installed_version=$(terraform-docs --version | head -n1 | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
