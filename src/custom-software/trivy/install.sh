...
# Vulnerability scanner for containers and code
...
install_trivy() {
    ...
    if command -v trivy >/dev/null 2>&1; then
        ...
        current_version=$(trivy --version | head -n1 | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    ...
    # Download trivy
    local download_url="https://github.com/aquasecurity/trivy/releases/download/${TOOL_VERSION}/trivy_${TOOL_VERSION:1}_Linux-64bit.tar.gz"
    local archive_file="$temp_dir/trivy.tar.gz"
    ...
    # Install binary
    local binary_path="$temp_dir/trivy"
    if [ ! -f "$binary_path" ]; then
        log_error "trivy binary not found in extracted archive"
        rm -rf "$temp_dir"
        return 1
    fi
    ...
    if ! sudo mv "$binary_path" "/usr/local/bin/trivy"; then
    ...
    if command -v trivy >/dev/null 2>&1; then
        ...
        installed_version=$(trivy --version | head -n1 | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
